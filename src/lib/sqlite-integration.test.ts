...
describe("SQLite Integration Tests", () => {
  ...
  beforeAll(async () => {
    ...
    const migrationSQL = readFileSync(
      path.join(process.cwd(), "migrations/sqlite/001_initial_schema.sql"),
      "utf8"
    );
  ...
  test("should query all users", async () => {
    const query = { User: {} };
    const result = await QueryService.exec(query, Model, db, "SQLite");
  ...
  test("should insert a new user", async () => {
    ...
    const result = await QueryService.mutate(mutation, Model, db, "SQLite");
  ...
  test("should delete a user certificate", async () => {
    ...
    const insertResult = await QueryService.mutate(insertMutation, Model, db, "SQLite");
    ...
    const deleteResult = await QueryService.mutate(deleteMutation, Model, db, "SQLite");
  ...
  test("should generate correct SQL for SQLite", () => {
    const query = {
      User: {
        projection: {
          firstName: true,
    ...
    const sql = QueryService.getSQL(query, Model, "SQLite");
  ...
  test("should handle batch inserts", async () => {
    ...
    const result1 = await QueryService.mutate(mutation1, Model, db, "SQLite");
    ...
    const result2 = await QueryService.mutate(mutation2, Model, db, "SQLite");
