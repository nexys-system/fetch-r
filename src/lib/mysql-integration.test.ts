...
describe("MySQL Integration Tests", () => {
  ...
  beforeAll(async () => {
    ...
    // First connect to default mysql database to create test database
    ...
    const migrationSQL = readFileSync(
      path.join(process.cwd(), "migrations/mysql/001_initial_schema.sql"),
      "utf8"
    );
  ...
  test("should query all users", async () => {
    const query = { User: {} };
    const result = await QueryService.exec(query, Model, db, "MySQL");
  ...
  test("should insert a new user", async () => {
    ...
    const result = await QueryService.mutate(mutation, Model, db, "MySQL");
  ...
  test("should delete a user certificate", async () => {
    ...
    const insertResult = await QueryService.mutate(insertMutation, Model, db, "MySQL");
    ...
    const deleteResult = await QueryService.mutate(deleteMutation, Model, db, "MySQL");
  ...
  test("should generate correct SQL for MySQL", () => {
    const query = {
      User: {
        projection: {
          firstName: true,
    ...
    const sql = QueryService.getSQL(query, Model, "MySQL");
    
    expect(sql).toBeDefined();
    expect(typeof sql).toBe("string");
    expect(sql.toLowerCase()).toContain("select");
    expect(sql.toLowerCase()).toContain("from user");
    expect(sql.toLowerCase()).toContain("join country");
  ...
  test("should handle batch inserts", async () => {
    ...
    const result1 = await QueryService.mutate(mutation1, Model, db, "MySQL");
    ...
    const result2 = await QueryService.mutate(mutation2, Model, db, "MySQL");
  ...
  test("should handle MySQL-specific SQL syntax differences", () => {
    // Test MySQL-specific features like backticks and LIMIT syntax
    const query = {
      User: {
        projection: {
          firstName: true,
    ...
    expect(sql).toBeDefined();
    expect(sql).toContain("LIMIT 2, 5"); // MySQL syntax (offset, limit)
    expect(sql).toContain('WHERE t0.`is_admin`=0'); // MySQL with backticks
    expect(sql).toContain('`'); // MySQL uses backticks
  });
});